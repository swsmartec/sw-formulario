...
export const FinancialForm = (props: any) => {
  ...
  React.useEffect(() => {
    // @ts-ignore
    setIsDependiente(values[economicActivity.name] == 'Dependiente');
    // @ts-ignore
    setIsInDependiente(values[economicActivity.name] == 'Independiente');
  }, [values]);
  ...
  React.useEffect(() => {
    const getData = async () => {
      ...
      const wsTypeOfActivity = await post(apiCatalogList, {
        Codigo: 'CAT_PER_ACV_ECO'
      });
      listTypeOfActivity = wsTypeOfActivity['CAT_PER_ACV_ECO'];
